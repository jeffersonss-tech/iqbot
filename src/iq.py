...
def load_goals(iq):
    ...
    inst = {i: profits_from_iq[i]['turbo'] for i in assets_from_iq['turbo']
            if assets_from_iq['turbo'][i]['open'] == True}
...
def login(IQ_api, balance_mode):
    ...
    try:
        ...
        if check:
            print('Logged in...')
            print('Tipo de saldo: ', balance_mode)
            iq.change_balance(balance_mode)
            print('Saldo: ', iq.get_balance())
    except:
